...
export default function AdminAnalyticsDashboard() {
    ...
    useEffect(() => {
        Promise.all([
            fetch('/api/analytics/admin/supply-chain-overview').then(r => r.json()),
            fetch('/api/analytics/admin/total-volume-moved').then(r => r.json()),
            fetch('/api/analytics/admin/regional-bottlenecks').then(r => r.json()),
            fetch('/api/analytics/admin/delivery-performance').then(r => r.json()),
            fetch('/api/analytics/admin/summary').then(r => r.json()),
        ]).then(([sc, vol, bottle, delivery, sum]) => {
            setSupplyChain(sc.data || []);
            setVolumeData(vol.data || {});
            setBottlenecks(bottle.data || []);
            setDeliveryPerf(delivery.data || []);
            setSummary(sum.data || {});
            setLoading(false);
        });
    }, []);
