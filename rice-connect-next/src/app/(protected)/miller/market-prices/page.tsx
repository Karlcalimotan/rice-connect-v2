...
export default function MillerMarketPricesPage() {
  ...
  useEffect(() => {
    const fetchPrices = async () => {
      ...
      if (res.ok) {
        ...
        setPrices(prices.map((p: Record<string, unknown>) => ({
          id: p.id,
          rice_variety: p.riceVariety,
          price_per_kg: p.pricePerKg,
          market_region: p.marketRegion,
            ...
            <div className="mt-8 rounded-lg border bg-white p-6 shadow-sm">
              ...
              <ResponsiveContainer width="100%" height={350}>
                <LineChart data={chartData}>
                  ...
                  <XAxis dataKey="date" tick={{ fontSize: 12 }} tickFormatter={(val: string) => formatDate(val)} />
                  <YAxis tick={{ fontSize: 12 }} tickFormatter={(val: number) => `₱${val}`} />
                  <Tooltip formatter={(value, name) => [formatCurrency(Number(value)), String(name)]} labelFormatter={(label) => formatDate(String(label))} />
                  <Legend />
                  {displayVarieties.map((variety, i) => (
                    <Line key={variety} type="monotone" dataKey={variety} stroke={COLORS[i % COLORS.length]} strokeWidth={2} dot={{ r: 4 }} />
                  ))}
                </LineChart>
              </ResponsiveContainer>
            </div>
